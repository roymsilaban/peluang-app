...
st.write("Aplikasi ini menyelesaikan SPL 2 & 3 variabel, serta menampilkan grafik garis untuk SPL 2 variabel.")
...
if jenis == "SPL 2 Variabel":
    ...
    if st.button("Hitung SPL 2 Variabel"):
        ...
        try:
            hasil = np.linalg.solve(A, B)  # titik perpotongan
            x_sol, y_sol = hasil[0], hasil[1]

            st.success(f"Solusi: x = {x_sol:.4f}, y = {y_sol:.4f}")

            # =====================
            # GRAFIK GARIS
            # =====================
            st.subheader("Grafik Garis SPL")

            x = np.linspace(-10, 10, 400)

            # Persamaan 1: a11 x + a12 y = b1 → y = (b1 - a11 x) / a12
            y1 = (b1 - a11 * x) / a12

            # Persamaan 2: a21 x + a22 y = b2 → y = (b2 - a21 x) / a22
            y2 = (b2 - a21 * x) / a22

            fig, ax = plt.subplots()
            ax.plot(x, y1, label="Persamaan 1", linewidth=2)
            ax.plot(x, y2, label="Persamaan 2", linewidth=2)

            # Titik perpotongan
            ax.scatter(x_sol, y_sol, color="red", s=80, label="Solusi (Titik Potong)")

            ax.set_xlabel("x")
            ax.set_ylabel("y")
            ax.set_title("Grafik SPL 2 Variabel")
            ax.legend()
            ax.grid(True)
